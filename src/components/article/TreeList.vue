    ...
    <div class="body-con clearfix">
      ...
      <div class="listTitle" :style="{'padding-left': (item.level-1)*paddingSize + 'px'}">
        ...
        <template v-else>
          ...
          <a class="m-l-md icon_lb_add" @click="addChild" title="添加分类"></a>
        </template>
      </div>
      <div>
        <el-switch
      ...
      <div class="listOpt">
        <router-link :to="'/article/category/edit/'+item.id" class="icon_l_edit" title="编辑"></router-link>
  ...
  export default{
    ...
    methods: {
      ...
      subAdd: function () {
        ...
        that.$http({
          method: 'post',
          url: '/article_category/addsubmit',
          params: {
