<template>
    <div id="recomData">
        <vue-form :state="formstate" @submit.prevent="onSubmit" class="form-horizontal m-t" id="commentForm">
            <validate class="form-group" v-for="(item,index) in fields.length" :key="index">                
                <div class="col-sm-3 posR">                                        
                    <span class="f-c-r">*</span>图片：<input type="text" :name="'img_url'+index" class="form-control picurl" v-model="fields[index].img_url" required :class="fieldClassName(formstate.img_url)">
