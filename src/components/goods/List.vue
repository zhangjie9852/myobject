                    ...
                    <div class="ibox float-e-margins">
                        ...
                        <div class="ibox-content">
                            <div class="form-inline m-b-sm clearfix">
                                ...
                                <span v-if="Lstatus==1">
                                <button type="button" class="btn btn-m btn-warning m-l-xs" v-if="isOnline==-1 ||isOnline==''" @click="batchOpt('up','上架')">上架</button>
                                <button type="button" class="btn btn-m btn-warning m-l-xs" v-if="isOnline==1 ||isOnline==''" @click="batchOpt('down','下架')">下架</button>
                                <button type="button" class="btn btn-m btn-success m-l-xs" @click="batchOpt('topic','置顶')">置顶</button>
                                <button type="button" class="btn btn-m btn-success m-l-xs" @click="batchOpt('untopic','取消置顶')">取消置顶</button>
							    <el-tooltip placement="top">
                                <button type="button" class="btn btn-m btn-primary m-l-xs" @click="batchOpt('del','删除')">批量删除</button>
                                <div slot="content">删除后的商品可在商品回收站保存。<br/>可以进行商品还原，也可以彻底删除</div>
                                </el-tooltip>
                                </span>
								<span v-else>
                                <button type="button" class="btn btn-m btn-primary m-l-xs" @click="batchOpt('restore','还原')">批量还原</button>                                
                                <button type="button" class="btn btn-m btn-warning m-l-xs" @click="batchOpt('callback','删除')">批量删除</button>
                                </span>
                                <router-link to="/goods/list/add" class="btn btn-m btn-danger hj_fr">添加商品</router-link>
                            </div>
                            <div class="btn-group m-b-sm">		                      
		                      <button type="button" class="btn m-r-xs" :class="isOnline==''&&Lstatus==1?'btn-primary':'btn-default'" @click="ListState('',1)">全部商品</button>
		                      <button type="button" class="btn m-r-xs" :class="isOnline==1?'btn-primary':'btn-default'" @click="ListState(1,1)">上架商品</button>
		                      <button type="button" class="btn m-r-xs" :class="isOnline==-1?'btn-primary':'btn-default'" @click="ListState(-1,1)">下架商品</button>
		                      <button type="button" class="btn m-r-xs" :class="Lstatus==-1?'btn-primary':'btn-default'" @click="ListState('',-1)">回收站</button>
		                    </div>
		                    <div class="table-box">                               
	                                ...
	                                <table class="table table-striped table-bordered table-hover">
	                                    ...
	                                    <tbody class="goodlist">                                    	
	                                        <tr v-for="(item,index) in clist">
	                                            ...
	                                            <td class="opt">
	                                            	<span class="opt-down" @click.stop="opToggle(clist,item.goods_id,'goods_id')">处理 <i class="fa fa-caret-down"></i></span>
	                                                <ul v-if="Lstatus==1" v-show="item.isOptShow"> 	
	                                                    <li><router-link :to="'/goods/list/edit/'+item.goods_id"><i class="icon_l_edit"></i> 编辑</router-link></li>
	                                                    ...
	                                                    <li><a @click="delOne(item.goods_id,'del')"><i class="icon_l_delete"></i> 删除</a></li>
	                                                </ul>
